                ...
                if dags_tgt is not None:
                    ...
                    if path is not None:
                        ...
                        if transitions:
                            ...
                            if pair_tgt:
                                targeted_policies = [
                                    p.get("policy_name") for p in pair_tgt if p
                                ]
                            print(
            ...
            rows.append(
                {
                    ...
                    "targeted_policies": json.dumps(targeted_policies)
                    if targeted_policies
                    else None,
                    "exhaustive_reward": exhaustive_reward,
                    "exhaustive_policies": json.dumps(exhaustive_policies)
                    if exhaustive_policies
                    else None,
                    "hybrid_reward": hybrid_reward,
                    "hybrid_policies": json.dumps(hybrid_policies)
                    if hybrid_policies
                    else None,
...
def _get_experiment_index_paths(
    index_path: str, metadata_path: str, experiment: str
) -> tuple[str, str]:
    """
    Get experiment-specific index paths if they exist, otherwise return original paths.

    Args:
        index_path: Base index path (e.g., "faiss_index/policy.index")
        metadata_path: Base metadata path (e.g., "faiss_index/metadata.pkl")
        experiment: Experiment type ("trivial", "double", or "triple")

    Returns:
        Tuple of (index_path, metadata_path) - experiment-specific if exists, else original
    """
    index_dir = os.path.dirname(index_path) or "faiss_index"
    index_name = os.path.basename(index_path)
    metadata_name = os.path.basename(metadata_path)

    exp_index = os.path.join(index_dir, experiment, index_name)
    exp_metadata = os.path.join(index_dir, experiment, metadata_name)

    # Use experiment-specific index if it exists
    if os.path.exists(exp_index) and os.path.exists(exp_metadata):
        print(f"Using experiment-specific index: {exp_index}")
        return exp_index, exp_metadata
...
def run_all_specs(
    ...
    for spec in specs:
        ...
        # Get experiment-specific index paths (falls back to main index if not found)
        trivial_index, trivial_meta = _get_experiment_index_paths(
            index_path, metadata_path, "trivial"
        )
        double_index, double_meta = _get_experiment_index_paths(
            index_path, metadata_path, "double"
        )
        triple_index, triple_meta = _get_experiment_index_paths(
            index_path, metadata_path, "triple"
        )
        ...
        run_full_experiment(
            ...
            index_path=trivial_index,
            metadata_path=trivial_meta,
        ...
        run_full_experiment(
            ...
            generate_plots=generate_plots,
            index_path=double_index,
            metadata_path=double_meta,
        ...
        run_full_experiment(
            ...
            generate_plots=generate_plots,
            index_path=triple_index,
            metadata_path=triple_meta,
...
def main():
    ...
    if args.mode in ("trivial", "all"):
        ...
        trivial_index, trivial_meta = _get_experiment_index_paths(
            args.index_path, args.metadata_path, "trivial"
        )
        run_full_experiment(
            output,
            args.seeds,
    ...
    if args.mode in ("double", "all"):
        ...
        double_index, double_meta = _get_experiment_index_paths(
            args.index_path, args.metadata_path, "double"
        )
        run_full_experiment(
            output,
            args.seeds,
    ...
    if args.mode in ("triple", "all"):
        ...
        triple_index, triple_meta = _get_experiment_index_paths(
            args.index_path, args.metadata_path, "triple"
        )
        run_full_experiment(
            output,
            args.seeds,
