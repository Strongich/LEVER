"""
Full experiment runner for DQN policy composition using GPI.

GPI (Generalised Policy Improvement):
  Q_combined(a) = max(Q1(s,a), Q2(s,a), ...)
  a* = argmax_a Q_combined(a)
...
# ── Grid-8 preset (matches train_dqn grid8) ─────────────────────────────────
GRID_SIZE = 8
NUM_BALLS = 6
NUM_WALLS = 4
NUM_LAVA = 4
...
# Reward defaults (matching train_dqn.py)
BALL_REWARD = 15.0
KEY_REWARD = 20.0
EXIT_REWARD = 50.0
...
def get_dqn_model(model_path: str, device: str = "cpu") -> DQN:
    """Load a DQN model, caching to avoid redundant disk reads."""
    if model_path not in _model_cache:
        _model_cache[model_path] = DQN.load(model_path, device=device)
...
def obs_from_state(
    ...
    Returns a (3, local_size, local_size) uint8 array ready for the DQN CNN.
    """
...
def gpi_action(models: list[DQN], obs: np.ndarray, device: str = "cpu") -> int:
    """GPI: max Q across models per action, then argmax."""
    obs_tensor = torch.as_tensor(obs[np.newaxis]).to(device)
    q_all = []
    for model in models:
        with torch.no_grad():
            q_values = model.q_net(obs_tensor)  # (1, n_actions)
            q_all.append(q_values)
    q_stack = torch.stack(q_all, dim=0)  # (n_models, 1, n_actions)
    q_combined = q_stack.max(dim=0).values  # (1, n_actions)
    return q_combined.argmax(dim=1).item()
...
def gpi_rollout_simulated(
    ...
    for _ in range(max_steps):
        ...
        action = gpi_action(models, obs, device)
...
# ── Evaluate a single DQN model on one seed ──────────────────────────────────
def evaluate_single_model(
    model: DQN, reward_system: str, seed: int, device: str = "cpu"
) -> float:
...
def evaluate_gpi_seed(
    models: list[DQN], reward_system: str, seed: int, device: str = "cpu"
) -> float:
    env = _make_eval_env(reward_system)
    obs, _ = env.reset(seed=seed)
    obs = _maybe_transpose_obs(obs)
...
def _load_candidate_model(
    ...
    if not model_path or not os.path.exists(model_path):
        return None
    try:
        return get_dqn_model(model_path, device)
...
def targeted_composition(
    ...
    device: str = "cpu",
) -> tuple[list[DQN] | None, list[str] | None, float]:
...
def exhaustive_composition(
    ...
    device: str = "cpu",
) -> tuple[list[DQN] | None, list[str] | None, float, float | None]:
    ...
    best_models: list[DQN] | None = None
    best_names: list[str] | None = None
    best_pred = -float("inf")

    for combo in combos:
        combo_models: list[DQN] = []
...
def run_dqn_experiment(
    ...
    eval_seeds_path: str = "deeprl_runs_dqn_try6/8/eval_env_seeds.json",
    experiments: list[dict] | None = None,
    policy_list: list[str] | None = None,
    index_path: str = "faiss_index_dqn/base/policy_dqn.index",
    metadata_path: str = "faiss_index_dqn/base/metadata_dqn.pkl",
    regressor_model_path: str = "models_dqn/reward_regressor_dqn_base.pkl",
    regressor_variant: str = "base",
    canonical_states_path: str = "data_rl/canonical_states_dqn_base.npy",
    setup_name: str = "base",
    hybrid_top_k: int = 3,
    ...
    """Run targeted / exhaustive / hybrid DQN composition experiments."""
...
def _build_mode_config(
    mode: str,
    faiss_base_dir: str = "faiss_index_dqn",
    ...
    return {
        ...
        "index_path": f"{faiss_base_dir}/{variant}/policy_dqn.index",
        "metadata_path": f"{faiss_base_dir}/{variant}/metadata_dqn.pkl",
        "regressor_model_path": f"{models_dir}/reward_regressor_dqn_{variant}.pkl",
        "regressor_variant": variant,
        "canonical_states_path": f"{data_dir}/canonical_states_dqn_{variant}.npy",
...
def run_all_modes(
    ...
    for mode in _MODE_VARIANTS:
        ...
        output_csv = os.path.join(results_dir, f"full_experiment_dqn_{mode}.csv")
        print(f"\n{'#' * 80}")
        print(f"# Mode: {mode}")
        print(f"{'#' * 80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run DQN policy composition experiments with GPI."
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    if args.mode == "all":
        ...
        output_csv = args.output or os.path.join(
            args.results_dir, f"full_experiment_dqn_{args.mode}.csv"
        )
