"""
DQN Training for LeverGrid Environment.
...
Usage:
    python -m policy_reusability.data_generation.deeprl.train_dqn --help
    python -m policy_reusability.data_generation.deeprl.train_dqn --timesteps 1000000
    python -m policy_reusability.data_generation.deeprl.train_dqn --all-rewards --sizes 16,32
"""
...
def _plot_loss_curves(progress_csv: Path, output_path: Path) -> None:
    """Plot DQN loss curves from SB3 progress.csv if available."""
    ...
    loss = _get_series("train/loss")
    q_values = _get_series("train/q_values")

    if not (loss or q_values):
    ...
    if loss:
        plt.plot(timesteps[: len(loss)], loss, label="loss")
    if q_values:
        plt.plot(timesteps[: len(q_values)], q_values, label="q_values")
...
def train_dqn(
    ...
    """
    Train a DQN agent on the LeverGrid environment.
    ...
    Args:
        ...
        train_freq: Frequency of training updates (in steps)
        gradient_steps: Gradient steps per update
        target_update_interval: Steps between target network updates
        exploration_fraction: Fraction of training for exploration annealing
    ...
    # No reward normalization for DQN (observations left raw).
...
def main():
    parser = argparse.ArgumentParser(
        description="Train DQN on LeverGrid environment",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Soft update coefficient (1.0 = hard update)",
    )
    parser.add_argument("--gamma", type=float, default=0.99, help="Discount factor")
    parser.add_argument(
        "--train-freq", type=int, default=4, help="Steps between training updates"
    )
    parser.add_argument(
        "--gradient-steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Fraction of training for epsilon annealing",
    )
    parser.add_argument(
