...
class GridWorld(gym.Env):
    ...
    @staticmethod
    def _lookup_dist(dist_map, pos, default=None):
        """Look up BFS distance, returning default for unreachable positions."""
        if dist_map is None:
            return default
        d = dist_map[pos[0], pos[1]]
        if d < 0:
            return default  # Unreachable position
        return int(d)

    @property
    def _max_dist(self):
        """Max distance proxy for unreachable positions."""
    ...
    def _get_reward(self, prev_agent_position):
        ...
        if use_gold:
            ...
            if remaining_golds:
                ...
                # Use _max_dist for unreachable positions to penalize dead-ends.
                prev_dist = self._lookup_dist(
                    self._dist_to_target, prev_agent_position, self._max_dist
                )
                curr_dist = self._lookup_dist(
                    self._dist_to_target, self.agent_position, self._max_dist
                )
                reward_total += prev_dist - curr_dist
        ...
        if use_lever:
            if self.lever_position is not None and not self.lever_collected:
                prev_dist = self._lookup_dist(
                    self._dist_to_lever, prev_agent_position, self._max_dist
                )
                curr_dist = self._lookup_dist(
                    self._dist_to_lever, self.agent_position, self._max_dist
                )
                reward_total += prev_dist - curr_dist
            elif not use_path:
                prev_dist = self._lookup_dist(
