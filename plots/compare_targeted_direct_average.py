"""
Plot average reward for direct targeted composition across X1/X5/X10.

Reads targeted_direct/<spec>/targeted_direct_results.csv and aggregates
scratch_reward vs direct_targeted_reward. This mirrors the style of
compare_compositions_average.py but focuses only on targeted composition.
"""
...
def load_spec_means(results_dir: str, spec: str):
    csv_path = os.path.join(results_dir, spec, "targeted_direct_results.csv")
    ...
    rewards = {
        ...
        "Targeted": _mean([m["Targeted"] for m in type_means]),
    }
    return rewards
...
def plot_average(results_dir: str, output_path: str, specs: list[str]):
    ...
    ax.set_title("Direct Targeted - Reward")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot average direct targeted results across X1/X5/X10."
    )
    parser.add_argument(
        "--results-dir",
        type=str,
        default="targeted_direct",
        help="Directory containing targeted_direct/<spec>/targeted_direct_results.csv",
    )
    parser.add_argument(
        "--specs",
        nargs="*",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="targeted_direct/average_targeted_direct.png",
