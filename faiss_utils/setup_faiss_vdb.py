...
class FaissVectorDB:
    ...
    def search_similar_policies(self, query_text, k=5, policy_seed=None, policy_spec=None):
        ...
        # Filter metadata: INCLUDE same seed, INCLUDE same spec
        if policy_seed is not None or policy_spec is not None:
            ...
            filtered_indices = [
                ...
                if (
                    ...
                    or str(meta.get("policy_seed")) == policy_seed_str  # INCLUDE same seed
                )
                and (
                    policy_spec_str is None
                    or str(meta.get("spec")) == policy_spec_str  # INCLUDE same spec
                )
            ]
