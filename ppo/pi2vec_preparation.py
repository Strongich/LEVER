"""
Preparation script for PPO policies using the pi2vec framework.

Analogous to tabular/pi2vec_preparation.py but for deep RL (PPO) trained policies
stored under deeprl_runs/8/.
...
def prepare_ppo_policies(
    ...
    """Main preparation function for PPO policies."""
    ...
    for setup_name, setup_rewards in SETUP_CONFIGS.items():
        ...
        cs_path = os.path.join(data_dir, f"canonical_states_ppo_{setup_name}.npy")
        np.save(cs_path, cs)
    ...
    for setup_name in SETUP_CONFIGS:
        ...
        idx_path = os.path.join(setup_dir, "policy_ppo.index")
        meta_path = os.path.join(setup_dir, "metadata_ppo.pkl")
        if reset:
    ...
    if skip_regressor:
        ...
        for setup_name in SETUP_CONFIGS:
            ...
            json_path = os.path.join(
                data_dir, f"regressor_training_data_ppo_{setup_name}.json"
            )
    ...
    for setup_name in SETUP_CONFIGS:
        ...
        model_path = os.path.join(
            models_dir, f"reward_regressor_ppo_{setup_name}.pkl"
        )
        plot_path = os.path.join(
            plots_dir, f"regression_plot_ppo_{setup_name}.jpeg"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare pi2vec assets for PPO policies (successor + FAISS + regressor)."
    )
    parser.add_argument(
        "--base-dir",
        type=str,
