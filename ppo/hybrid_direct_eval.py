"""Evaluate best-case hybrid composition by trying top-k PPO snapshots per sub-policy via GPI."""
...
from ppo.full_experiment import (
    GRID_SIZE,
    LOCAL_SIZE,
    OBS_MODE,
    evaluate_gpi_seed,
...
def run_hybrid_direct(
    ...
    for exp in EXPERIMENTS:
        ...
        for task_name in subtasks:
            ...
            if not paths:
                ...
                valid = False
                break
            models = [get_ppo_model(p, device) for p in paths]
    ...
    # Write CSV
    output_csv = os.path.join(output_dir, "hybrid_direct_ppo_results.csv")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate best-case hybrid PPO composition via GPI (cross-product of top-k snapshots)."
    )
    parser.add_argument(
        "--base-dir",
        type=str,
